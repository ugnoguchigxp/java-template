...
3. `UserRepository` and `RefreshTokenRepository` hide MyBatis mapper details. The active mapper XML targets PostgreSQL and the migration runner reads `db/migration`.
...
The Java 25 PostgreSQL variant uses Gradle 9.1.0, Spring Boot 4.0.6, Spring Framework 7, Spring Security 7, MyBatis Spring Boot Starter 4.0.0, and the PostgreSQL JDBC driver. Java records are used for immutable authentication response values while mutable request and persistence models retain bean accessors for validation and MyBatis mapping.
